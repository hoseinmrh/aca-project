...
def topk(x, ratio, batch):
    ...
    for i in range(batch_size):
        start = cum_num_nodes[i]
        end = cum_num_nodes[i] + num_nodes[i]
        values = x[start:end]
        _, idx = values.topk(k[i].item())
        perm.append(idx + start)
    return torch.cat(perm, dim=0)

def filter_adj(edge_index, edge_attr, perm, num_nodes=None):
    """Filters edges and edge attributes for the pooled subgraph."""
    mask = (edge_index[0].unsqueeze(1) == perm).any(dim=1) & (edge_index[1].unsqueeze(1) == perm).any(dim=1)
    edge_index = edge_index[:, mask]
    if edge_attr is not None:
        edge_attr = edge_attr[mask]
    if num_nodes is not None:
        idx = perm.new_full((num_nodes,), -1)
        idx[perm] = torch.arange(perm.size(0), device=perm.device)
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        if self.cached and self.cached_result is not None:
            if edge_index.size(1) != self.cached_num_edges:
                raise RuntimeError('Cached {} number of edges, but found {}'.format(self.cached_num_edges, edge_index.size(1)))
...
# Add fallback for spspmm and coalesce if torch_sparse is not available
# (for now, raise NotImplementedError if used)
def spspmm(*args, **kwargs):
    raise NotImplementedError("spspmm requires torch_sparse. Please install or refactor this part.")
def coalesce(*args, **kwargs):
    raise NotImplementedError("coalesce requires torch_sparse. Please install or refactor this part.")
